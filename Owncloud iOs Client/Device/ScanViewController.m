...
- (void)viewDidLoad
{
    ...
    [self setupScanLine];
    self.navigationItem.title = NSLocalizedString(@"序列号扫描", nil);
...
-(void)setupIntroductionLable
{
    ...
    labIntroudction.text=NSLocalizedString(@"将设备机身或说明书上的二维码置于矩形框内即可自动识别", nil);
...
-(CGRect)makeScanReaderRect
{
    ...
    CGRect scanRect = CGRectMake(0, 64, scanSize, scanSize);
...
-(void)scanComplete:(NSString*)deviceID
{
    ...
    [[DeviceManager sharedManager] getDeviceName:deviceID completionBlock:^(NSString *name, NSError *error) {
        ...
        if (strongSelf) {
            if (name != nil) {
                ...
                addDeviceViewController.deviceID = deviceID;
                addDeviceViewController.deviceName = name;
